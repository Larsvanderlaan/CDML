...
logit_probs <- plogis(X %*% rep(0.3, 5))  # Linear predictor for logistic model
A <- rbinom(n, 1, logit_probs)

# True outcome model
Y <- as.vector(2 * A + X %*% rep(0.5, 5) + rnorm(n))

# Predicted outcome regressions (for demonstration, using true model)
mu1 <- 2 + X %*% rep(0.5, 5)  # Expected outcome if A = 1
mu0 <- X %*% rep(0.5, 5)       # Expected outcome if A = 0
